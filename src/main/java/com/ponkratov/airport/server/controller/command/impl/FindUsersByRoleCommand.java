...
public class FindUsersByRoleCommand implements ActionCommand {
    private static final Logger LOG = LogManager.getLogger();

    @Override
    public CommandResult execute(Map<String, String> params) throws JsonProcessingException {
        UserService service = UserServiceImpl.getInstance();
        int roleID = Integer.parseInt(params.get(RequestAttribute.SEARCHCONDITION));
        List<User> users = null;
        try {
            users = service.findByRole(roleID);
